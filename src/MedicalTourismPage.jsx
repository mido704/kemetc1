...
export default function MedicalTourismPage({ user, onBack }) {
  ...
  const addHotel = () => { if(newHotel.name){ setHotels([...hotels,newHotel]); setNewHotel({name:"",type:"شقة",price:0,area:"",rooms:"",videoUrl:"",desc:""}); }};
  const addCar = () => { if(newCar.name){ setCars([...cars,newCar]); setNewCar({name:"",type:"سيدان",price:0,seats:"",ac:"",imgUrl:"",desc:""}); }};
        ...
        <div style={S.section}>
          ...
          {hotels.map((h,i) => (
            <div key={i}>
              <div style={S.card(selectedHotel===i,"#1A7FA8")} onClick={()=>!adminMode && setSelectedHotel(selectedHotel===i?null:i)}>
                {h.videoUrl
                  ? <video src={h.videoUrl} style={{width:"100%",height:180,objectFit:"cover",display:"block"}} controls />
              ...
              {adminMode && (
                <div style={S.adminSection}>
                  ...
                  <input style={S.inp} placeholder="رابط الفيديو (MP4 أو YouTube)" value={h.videoUrl} onChange={e=>updateHotel(i,"videoUrl",e.target.value)} />
          ...
          {adminMode && (
            <div style={{...S.adminSection,border:"2px dashed #1A7FA8"}}>
              ...
              <input style={S.inp} placeholder="رابط الفيديو" value={newHotel.videoUrl} onChange={e=>setNewHotel({...newHotel,videoUrl:e.target.value})} />
              ...
              <button style={S.btnSm} onClick={addHotel}>➕ إضافة</button>
            </div>
          )}
        </div>
        ...
        <div style={S.section}>
            ...
            <div key={i}>
              ...
              {adminMode && (
                <div style={S.adminSection}>
                  ...
                  <input style={S.inp} placeholder="رابط الصورة" value={car.imgUrl} onChange={e=>updateCar(i,"imgUrl",e.target.value)} />
          ...
          {adminMode && (
            <div style={{...S.adminSection,border:"2px dashed #2EAE8A"}}>
              ...
              <input style={S.inp} placeholder="رابط الصورة" value={newCar.imgUrl} onChange={e=>setNewCar({...newCar,imgUrl:e.target.value})} />
              ...
              <button style={S.btnSm} onClick={addCar}>➕ إضافة</button>
            </div>
          )}
        </div>
        ...
        <div style={S.section}>
          ...
          {totalPrice > 0 && (
            <div style={S.totalBox}>
              <div style={{color:"rgba(255,255,255,0.8)",fontSize:13}}>إجمالي المقدم (١٠٪)</div>
          ...
          {[
            ...
            {id:"instapay",icon:"⚡",name:"InstaPay",desc:"إنستا باي"},
          ].map(m=>(
          ...
          {!submitted
            ? <button style={S.btn} onClick={()=>{ if(disease.trim()&&payMethod) setSubmitted(true); }}>🏥 تأكيد الحجز وإرسال الطلب</button>
            : <div style={{background:"#F0FBF7",border:"2px solid #2EAE8A",borderRadius:20,padding:30,textAlign:"center",marginTop:8}}>
                ...
                <div style={{color:"#1A3A4A",fontSize:14,marginTop:10,lineHeight:1.8}}>سيتواصل معك فريقنا خلال <strong>٢٤ ساعة</strong></div>
                <div style={{marginTop:16,color:"#1A7FA8",fontWeight:700}}>📞 01000000000</div>
              </div>
          }
        </div>
