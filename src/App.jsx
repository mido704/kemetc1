  ...
  return (
    <div className='tour-card' onClick={()=>onBuy(tour)} style={{cursor:'pointer'}}>
      <div style={{ background:'linear-gradient(135deg,#0D0A02,#1A1200)', padding:'22px 16px', textAlign:'center', position:'relative', minHeight:160, overflow:'hidden' }}>
        {tour.image_url && <img src={tour.image_url} style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',objectFit:'cover'}} />}
        ...
        {!tour.image_url && <div style={{fontSize:58,marginBottom:6,position:'relative',zIndex:1}}>{tour.image_emoji||'🏛️'}</div>}
        {!tour.image_url && tour.rating>0 && <div style={{color:'var(--g)',fontSize:12,position:'relative',zIndex:1}}>{'⭐'.repeat(Math.floor(tour.rating||0))} {tour.rating||0} ({tour.reviews_count||0})</div>}
      </div>
      ...
      <div style={{ background:'linear-gradient(135deg,#0D0A02,#1A1200)', borderRadius:14, overflow:'hidden', marginBottom:16, position:'relative' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:10 }}>
          <div>
            ...
            {tour.rating>0 && <div style={{ color:'var(--g)', fontSize:13 }}>{'⭐'.repeat(Math.floor(tour.rating))} {tour.rating} ({tour.reviews_count} {t('تقييم','reviews',lang)})</div>}
          </div>
        ...
        <div>
          ...
          {followers.map(f=>(
            <div key={f.id} className='post-card' style={{display:'flex',gap:10,alignItems:'center'}}>
              <Avatar emoji={f.avatar_emoji||'👑'} size={44} url={f.avatar_url} />
              <div style={{flex:1}}><div style={{fontWeight:700,color:'var(--g)',fontSize:14}}>{f.nickname}</div><div style={{fontSize:12,color:'var(--tm)'}}>{f.name}</div></div>
              <button className={'btn '+(followedBack[f.id]?'btn-gh':'btn-g')} style={{fontSize:12,padding:'6px 12px'}} onClick={async()=>{ await usersAPI.follow(f.id); setFollowedBack(p=>({...p,[f.id]:true})); }}>{ followedBack[f.id] ? '✓ '+t('متابَع','Following',lang) : '+ '+t('تابع','Follow',lang)}</button>
            </div>
          ))}
        </div>
      )}
...
function EgyptNewsTab({ lang }) {
  ...
  const translateNews = async (newsItem, targetLang) => { setShowLangMenu(null); setTranslatingId(newsItem.id); try { const r = await fetch('https://api.anthropic.com/v1/messages', {method:'POST',headers:{'Content-Type':'application/json','x-api-key':import.meta.env.VITE_KEY,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:1000,messages:[{role:'user',content:'Translate to '+targetLang+' only, keep structure:\nTitle: '+newsItem.title_en+'\nSummary: '+newsItem.summary_en+(newsItem.content_en?'\nContent: '+newsItem.content_en:'')}]})}); const d=await r.json(); setTranslatedNews(prev=>({...prev,[newsItem.id]:d.content?.[0]?.text||''})); } catch(e){} setTranslatingId(null); };
