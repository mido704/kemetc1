...
function EgyptNewsTab({ lang }) {
  ...
  const translateNews = async (newsItem, targetLang) => { setShowLangMenu(null); setTranslatingId(newsItem.id); try { const r = await fetch('https://api.anthropic.com/v1/messages', {method:'POST',headers:{'Content-Type':'application/json','x-api-key':import.meta.env.VITE_KEY,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:1000,messages:[{role:'user',content:'Translate to '+targetLang+' only: '+newsItem.title_en+' '+newsItem.summary_en}]})}); const d=await r.json(); setTranslatedNews(prev=>({...prev,[newsItem.id]:d.content?.[0]?.text||''})); } catch(e){} setTranslatingId(null); };
